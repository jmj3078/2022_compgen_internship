...
if [ ! -d $INPUT\_varscan2 ];then
    mkdir $INPUT\_varscan2
    echo "make $INPUT\_varscan2 directory"
else
    echo "$INPUT\_varscan2 directory already exist"
fi
...
--output-snp ./$INPUT\_varscan2/$INPUT.output.snp \
--output-indel ./$INPUT\_varscan2/$INPUT.output.indel \
