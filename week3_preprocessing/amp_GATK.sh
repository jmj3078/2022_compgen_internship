#!/usr/bin/bash
#SBATCH -J amp_GATK_%j
#SBATCH -n 1
#SBATCH -N 1
#SBATCH -t 14-00:00
#SBATCH -p 48core
#SBATCH --mem=17G
#SBATCH -o ./log/amp_GATK_%j.out
#SBATCH -e ./log/amp_GATK_%j.err
...
#bwa mem $REF $FASTQ1 $FASTQ2 | samtools view -b -o $BAM.bam
...
date
echo "add read groups ..done, skip : mark duplicates"
...
-R $REF \
-o $BAM.sorted.RGadded.bam.list \
-I $BAM.sorted.RGadded.bam \
-dt NONE \
-nt 8
...
-R $REF \
-T IndelRealigner \
-targetIntervals $BAM.sorted.RGadded.bam.list \
-o $BAM.sorted.RGadded.realigned.bam \
...
INPUT=$BAM.sorted.RGadded.realigned.bam \
OUTPUT=$BAM.sorted.RGadded.realigned.fixed.bam \
SO=coordinate \
VALIDATION_STRINGENCY=LENIENT \
CREATE_INDEX=true
...
-T BaseRecalibrator \
-R $REF \
-I $BAM.sorted.RGadded.realigned.fixed.bam \
--knownSites $dbSNP \
-o $BAM.sorted.RGadded.realigned.fixed.bam.recal_data.grp
...
-R $REF \
-I  $BAM.sorted.RGadded.realigned.fixed.bam \
-BQSR  $BAM.sorted.RGadded.realigned.fixed.bam.recal_data.grp \
-o $BAM.sorted.RGadded.realigend.fixed.recal.bam
